...
MqlNet INet;
 ...
 extern lotType LotType = DYNAMIC; // Lot Type
extern string hostIp = "localhost"; // Socket [HOSTNAME/IP]  
extern int hostPort = 8080; // Port [8080]
// OFFICIAL INPUTS
extern string TradeComment = "MONKEYONE";
extern int MagicNumber = 0;
extern int Slippage = 10;
extern double Lots = 0.01;
 double Divider = 66;   
 uint BasketTime = 600;
 double BasketAmount = 0.001;
  int HistoryTrades = 23;
  double Multiplier = 9;
  double ProfitRatio = 1.14;
  double RSIDiff = 45;
extern string Symbols = "AUDCAD,AUDCHF,AUDJPY,CADCHF,CADJPY,CHFJPY,EURAUD,EURCAD,EURCHF,EURGBP,EURJPY,EURUSD,GBPAUD,GBPCAD,GBPCHF,GBPUSD,GBPJPY,NZDCAD,NZDJPY,NZDUSD,USDCAD,USDCHF,USDJPY"; // Reverse Pairs
//string Multipliers = "1.5,1.9,2.2,2.4,2.5,2.5,2.6,2.8,3.2,4"; 
  int MaxOpenSymbols = 3; // Open Pairs
  int MaxSignals = 3; // Max Signals
  int MaxTrades = 13; // Max Trades 
 ...
 int VelocitySignal = 11; // Entry Signal 
  int OrderDistance = 7; // Order Distance 
  int StopLoss = 465; // Stop Loss 
  int TrailingGap = 17; // Trailing Gap 
 int TrailingTarget = 23; // Trailing Target 
  int DeleteOrder = 15; // Delete Order 
  uint DeleteTime = 23; // Delete Time
      ...
      for( int pos = OrdersTotal() - 1; pos >= 0; pos-- ) { 
         ...
         switch ( OrderType() ) { 
            ...
            case OP_BUY:    
               ...
               if( this.info.bid - oop < this.info.largestLoss ){
                  this.info.largestLoss = this.info.bid - oop;
                  this.info.largestLossTicket = ot;
                  if( op < largeLoss ){
                     largeLoss=op;
                     largeLossTicket=this.info.largestLossTicket;
                  }
               }
            ...
            case OP_SELL:    
               ...
               if( oop - this.info.ask < this.info.largestLoss ){
   ...
   int manage(){
     ...
     double buyDist, sellDist;
      for( int pos = OrdersTotal() - 1; pos >= 0; pos-- ) { 
         ...
         switch ( OrderType() ) { 
           case OP_BUYSTOP: 
               if( this.info.velocity > this.info.velocitySignal * this.info.point && this.info.ask < oop - ( this.info.deleteOrder * this.info.point ) ){
            ...
            break;
            case OP_SELLSTOP: 
               if( this.info.velocity <= -this.info.velocitySignal * this.info.point && this.info.bid > oop + (this.info.deleteOrder * this.info.point ) ){
            ...
            case OP_BUY:
               ...
               if( largeLoss + this.account.basketProfit > AccountBalance() * BasketAmount && ot == largeLossTicket ){
 ...
 double make_request( double balance, double equity, double profit, string trades )
{
	...
	if( lastReq != reqest || (uint)TimeCurrent() - lastSend > 4 ){
